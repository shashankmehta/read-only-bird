...
EXISTING_AUTH_TOKEN=""
EXISTING_CT0=""
EXISTING_SECONDARY_AUTH_TOKEN=""
EXISTING_SECONDARY_CT0=""
...
if [[ -f "$REPO_DIR/.env" ]]; then
  ...
  EXISTING_SECONDARY_AUTH_TOKEN="${SECONDARY_AUTH_TOKEN:-}"
  EXISTING_SECONDARY_CT0="${SECONDARY_CT0:-}"
...
echo "-- Main account (for bookmarks, likes, home, mentions, lists) --"
read -rp "TWITTER_AUTH_TOKEN [${EXISTING_AUTH_TOKEN:+****}]: " INPUT_AUTH_TOKEN
read -rp "TWITTER_CT0 [${EXISTING_CT0:+****}]: " INPUT_CT0
echo ""
echo "-- Secondary account (for generic reads: search, tweets, profiles) --"
read -rp "SECONDARY_AUTH_TOKEN [${EXISTING_SECONDARY_AUTH_TOKEN:+****}]: " INPUT_SECONDARY_AUTH_TOKEN
read -rp "SECONDARY_CT0 [${EXISTING_SECONDARY_CT0:+****}]: " INPUT_SECONDARY_CT0
echo ""
...
SECONDARY_AUTH_TOKEN="${INPUT_SECONDARY_AUTH_TOKEN:-$EXISTING_SECONDARY_AUTH_TOKEN}"
SECONDARY_CT0="${INPUT_SECONDARY_CT0:-$EXISTING_SECONDARY_CT0}"
...
if [[ -z "$SECONDARY_AUTH_TOKEN" || -z "$SECONDARY_CT0" ]]; then
  echo "Error: SECONDARY_AUTH_TOKEN and SECONDARY_CT0 are required."
  exit 1
fi
